...
import { addMessage, deleteMessage, listenToChat } from "../../../../cont/firebase/chat/chatDB";
...
export default function Chat(vnodeInit: Vnode<Attrs, State>) {
  ...
  return {
    ...
    view: (vnode: Vnode<Attrs, State>) => {
      const messages = store.chat.messages.filter(msg=>msg.groupId === consultation.id).sort((a,b)=>a.created.seconds - b.created.seconds)
      return (
        <div class="chat section">
          <main>
            {messages.map(msg=><p key={msg.id}>{msg.message} <button onclick={()=>{handleDeleteMessage(msg.id)}}>Delete</button></p>)}
          </main>
